...
level_2 = np.where(level_1 == 1, np.random.choice([1, 0], size=num_sessions, p=[0.7, 0.3]), 0)  # 70% of L1 reach L2
level_3 = np.where(level_2 == 1, np.random.choice([1, 0], size=num_sessions, p=[0.5, 0.5]), 0)  # 50% of L2 reach L3
level_4 = np.where(level_3 == 1, np.random.choice([1, 0], size=num_sessions, p=[0.3, 0.7]), 0)  # 30% of L3 reach L4
level_5 = np.where(level_4 == 1, np.random.choice([1, 0], size=num_sessions, p=[0.2, 0.8]), 0)  # 20% of L4 reach L5

# Сведение данных в датафрейм.
df = pd.DataFrame({
    'user_id': user_ids,
    'device': np.random.choice(['iOS', 'Android', 'Chrome', 'Internet Explorer', '']),
